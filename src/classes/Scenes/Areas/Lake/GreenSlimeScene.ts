...
export class GreenSlimeScene extends AbstractLakeContent {
    ...
    private serviceLowCorruption(): void {
        ...
        this.outx("You sit next to the blob and gently touch its throbbing member.  The surface membrane is moist and almost velvety in texture, not entirely smooth but not truly rough either.  You slowly run a hand along its length, glancing at the creature's face as you do so.  It seems calmer somehow, more in control of its breathing.\n\n");
        // Big peoples!
        if (this.player.tallness >= 82) {
            this.outx("You try gently wrapping one of your massive hands around its erection, and squeezing.");
        }
        // Not so big peoples
        else {
            this.outx("You try to wrap one of your hands around the erection, but end up having to use both to get a firm grip.");
        }
        this.outx(
    ...
    private serviceLowCorruptionHighLust(): void {
        ...
        this.outx("You straddle the creature's chest and playfully run a hand over the tip of its featureless member, which is moist and soft to the touch.");
        // Tall peeps
        if (this.player.tallness >= 82)
            this.outx("  You grip the head of the member firmly with one of your large hands and tease the tip with your thumb for a moment before squeezing and sliding your hand to its base.");
        // Short peeps
        else
            this.outx("  You run a finger up and down the creature's member a few times before tightly wrapping your hands around the tip and running them to its base.");
        this.outx("  The erection is spongy and pliable to the touch, and with every pulse it seems to fill the cracks between your fingers.\n\n");
        ...
        if (this.player.vaginas.length > 0) {
            ...
            if (this.player.vaginalCapacity() <= 18)
                this.outx("  On top of that, you are unsure if the thick, foot and a half long appendage would even fit inside of you.");
        ...
        // Next PG
        // Get it hard
        this.outx("As you continue to work the creature's mass its erection gains more and more definition, slowly but surely turning into an unmistakable human penis.  ");
        // And here begins the giant goddamn height split.  Hoo boy.
        // Smaller characters!
        if (this.player.tallness < 82) {
            // Hand gets coated in sensitive slime\n
            this.outx("A light green fluid begins to coat your hand as you stroke the slime's cock, making your fingers feel warm and sending shivers of arousal through your entire body.\n\n");
            ...
            // female or herm version (if the herm didn't trigger male)
                ...
                if (this.player.cockTotal() > 0) {
                    ...
                    if (this.player.cockTotal() > 1) {
                        ...
                        if (this.player.biggestTitSize() == 0)
                            this.outx("You delicately grasp the head of one of your cocks, your hand slick with the fluids of yourself and of the creature.  The combination of the creature's movements and your own quickly overwhelms you.");
                    }
                    // Single prick
                    else {
                        this.outx(
                ...
                this.outx("When you open your eyes you notice that the creature's erection has disappeared and the almost predatory calm it possessed when you first encountered it has returned, though different somehow.");
                // New PG
                this.outx("\n\n");
            }
            ...
            this.outx("After a few moments the creature slides out from under you, slinking back into the waters of the lake as you recover from the ordeal.  When your wits return, there is nothing to do but stop back by camp.\n");
        }
        // PHEW! that was a big one.  now...  we do it all again for large players, whee! and this is why this thing is so goddamn long.
            ...
            // Female/Herm Masturbation
                ...
                this.outx("Eventually your own arousal becomes unbearable and your other hand finds its way to your ");
                // TITTY ACTIOOOON
                if (this.player.breastRows.length > 0 && this.player.biggestTitSize() > 0) {
                    this.outx(
                    ...
                    this.outx("A bead of realization dawns on you just before you feel it hit, the surprise making you squeeze and twist at the head of the slime's cock.  ");
                }
                // Non titty foreplay
                else
                    this.outx(
                ...
                if (this.player.cockTotal() > 0) {
                    ...
                    if (this.player.biggestTitSize() == 0 || this.player.breastRows.length == 0) {
                        ...
                        // SURPRIZES!
                        this.outx("as it does.  What comes next is an utter surprise, making you unconsciously wring the head of the slime's cock.  ");
                    }
                    // Clit engulph'ed!
                    this.outx(
                ...
                // Fems
                    ...
                    this.outx("Even as it does so it continues to masterfully work your body, putting you on edge with its ministrations and keeping you there for several torturous, glorious minutes as you practically lose control over your body.  Eventually the creature seems to pull on everything at once, finally causing you to orgasm as you all but scream in pleasure.  ");
                // Creature exits stage left!
                this.outx("You barely notice as the creature's erection recedes and it slides out from under you, leaving you utterly satisfied.  When you do finally recover your senses you find no other traces of its presence.");
            }
        }
        this.player.orgasm();
        this.dynStats("sen", 3);
    ...
    private maleRapesOoze(): void {
        ...
        this.outx("You approach the creature and examine it, finding no real orifices to speak of.  ");
        // Random check here for oral or anal
        // oral rape text
        if (GreenSlimeScene.rand(2) == 0) {
            // MULTICAWK
            if (this.player.cockTotal() > 1)
                this.outx("Shrugging, you decide to go with the closest thing available and walk towards its head.  You spend a moment deciding which of your organs to ravage the creature with, eventually settling on the largest,  ");
            else
                this.outx(
            ...
            // Continue the setup...
            this.outx("You pull back for a moment and aim your penis downward, resting the ");
            // !! PENISTYPE
            ...
            this.outx(" and grunt approvingly.  You begin to slowly pull yourself out and then thrust yourself back in, feeling a moistness as some fluid surrounds your cock.  ");
            // player doesn't care about monster's wellbeing
            if (this.player.cor >= 60)
                this.outx("You wonder for a moment if you've broken the creature's membranous skin, then shrug and decide you don't care.");
            // player shows concern for monster
            else
                this.outx("You wonder for a moment if you've broken the creature's membranous skin, but the continuing presence of the creature's throbbing erection implies that it's in relatively little distress.");
            // New PG
            this.outx("\n\n");
            ...
            this.outx(".  One of its arms reaches up to meekly rub at its own erection.  You grin as this fuels your own lust, and you start to thrust even harder.");
            ...
            // New PG for cumming!
            this.outx("\n\n");
            this.outx("Before long you find yourself shouting as you pump your seed into the creature, watching as the semen is caught by unseen currents and dispersed into its body.");
            // vaginal orgasm text
            if (this.player.vaginas.length > 0)
                this.outx(
        ...
        // anal rape text
            ...
            // Multipricks
            if (this.player.cockTotal() > 1) {
                this.outx("You shrug at the lack of obvious orifices and decide to go for the most common one, choosing the ");
            ...
            this.outx("  You position your member around where you think the asshole would be and push, smiling as the membrane moves aside and you penetrate into a tight, deep hole.  ");
            ...
            this.outx("  With a satisfied smile you begin pumping, slowly at first before picking up speed.  The slime's anus feels better than anything you've ever had before, apparently coming with its own lubrication but still being tight, hugging your cock like a custom-fit fuck toy.  ");
            // vaginal arousal text
            if (this.player.vaginas.length > 0)
                this.outx(
                    `Your ${this.vaginaDescript(
            ...
            this.outx("After only a few minutes you realize that you can't hold out for much longer, as whatever is inside the creature seems to be making you extra sensitive.  Deciding to make the most of it, you double your speed and grit your teeth, enjoying the slime for all it's worth before ");
            if (this.player.cockTotal() > 1)
                this.outx(
                ...
                this.outx("exploding inside it, your load filling the space around your cock with a white, milky fluid.");
            // vaginal orgasm text
            if (this.player.vaginas.length > 0)
                this.outx(
    ...
    private femaleRapesOoze(): void {
        this.player.slimeFeed();
        this.outx("You feel a stirring inside your feminine side as you eye the slime's throbbing erection, and decide to take advantage of its current state to satisfy your own urges.  You strip off your clothes and walk forward, straddling the creature's thighs and running a hand over its member.  You pull it in against your body and stroke the soft, velvety, and just slightly moist shaft,");
        if (this.player.cocks.length > 0) {
            this.outx(
        ...
        if (this.player.vaginas[0].virgin) {
            ...
            if (this.player.cor <= 20) {
                this.outx("  You hesitate for a moment as you hover over the creature, a sudden surge of nervousness comes over you.  This is a moment that only comes once.  You let your weight push the creature into you, slowly at first, then forcing yourself down as far you can go.  You gasp at the initial pain, but it quickly dissipates as the creature's fluids coat your interior with a cool, soothing fluid.  ");
            }
            // higher corruption tosses it aside
            else {
                this.outx("  You smile as you ram yourself onto the creature, finally removing your virginity.  You quickly start moving and give a pleased gasp as the creature's fluids coat your interior with a cool, soothing fluid.  ");
            }
        } else if (this.player.vaginalCapacity() >= 18) {
            this.outx("  You enthusiastically lower yourself onto the slime's cock, letting out a small gasp of surprise as it slides inside of you.  The creature quickly understands your intentions and reaches up with its hands, beginning to pump you furiously.  ");
        } else {
            this.outx("  You hesitate at the sheer size of the slime's cock and the creature, sensing your hesitation, reaches up with its strong arms and grips your ass, forcefully pushing you onto itself.  ");
        }
        this.outx(
        ...
        if (this.player.cockTotal() > 0) {
            if (this.player.cockTotal() == 1) {
                ...
                this.outx("  Your cock grows hard as you are penetrated, and with the creature handling the movement you reach down, jerking off a different cock with each hand.");
            }
        }
        this.outx("\n\n");
        this.outx(
        ...
        if (this.player.cockTotal() > 0) {
            if (this.player.cockTotal() > 1) {
                ...
                this.outx(" thick streams of semen onto you and the creature, your mind blanking at the massive relief.  You vaguely notice your ejaculate sinking into the creature's body and getting absorbed as you give yourself to pleasure.  ");
            } else {
                this.outx(
                    `Your ${this.cockDescript(
                        0
    ...
    private oozeButtRapesYou(): void {
        ...
        if (this.player.analCapacity() < 18 || this.player.ass.analLooseness == 0) {
            ...
            if (this.player.cor <= 50)
                this.outx("You feel a bit of fear as you tremble under the creature's weight, worried about the damage that its massive size might do to your ass.  ");
            else if (this.player.cor < 100)
                this.outx("In spite of your worries about the creature's size, you can't help but feel a little bit excited at the idea of having this beast take you from behind.  ");
            // and something special for the total whorebags that have somehow not already wrecked their asses.
            else
                this.outx("You almost lick your lips in anticipation and push your hips back greedily, eager to feel the monster's cock stretch your asshole wide.  ");
        }
        this.outx("You grimace slightly and it pushes at your sphincter, then penetrates.  You let out a slight gasp in spite of yourself as its cock warps, deforming to stretch your ass without pushing you beyond your limits.  It pushes itself several inches inside you and stops, pausing as you catch your breath and collect yourself.");
        // De-virgin
        this.player.buttChange(this.monster.cockArea(0), true);
        this.outx("\n\n");
        this.outx("With a deliberate slowness the slime pulls out of you and then thrusts back inside, this time going deeper than before.  It continues like this and you realize that the penetration is entirely painless, the creature's pliability and moistness allowing it to easily glide inside you.  Before long you feel it bottom out inside you, its soft member curving perfectly through your insides as it trunk presses against your behind.  The slime begins fucking you with short, shallow strokes and you feel a moistness build inside your ass as the creature's fluid begins to accumulate.  It begins to quicken and deepen its strokes as you feel a heat build inside of you, the creature's fluid apparently having an aphrodisiac effect.\n\n");
        this.outx("Before long you find yourself moaning under the beast's attack, pleasure washing through your nethers in spite of yourself as it pumps your ass.  ");
        // cock arousal
        if (this.player.cockTotal() > 0)
            this.outx("You feel pangs of pleasure run through your body as its member repeatedly strokes your prostate from the inside, stimulating it.  ");
        this.outx("Its dick shifts slightly, continually growing and shrinking in tiny amounts in a continual, heavy throb.  ");
        // masturbation begin
        // Cawkz!
        if (this.player.cockTotal() > 0) {
            // Single
            if (this.player.cockTotal() == 1)
                this.outx("You quickly grow erect under the creature's embrace as it pistons into your ass, and before long you stand at full attention.  You weakly rub yourself with one hand as it pumps you, and quickly approach orgasm.  ");
            // Multi!
            else
                this.outx(
    ...
    private oozeRapesYouOrally(): void {
        ...
        if (this.player.cor >= 70) {
            ...
            if (this.player.vaginas.length > 0) {
                ...
                this.outx("grows wet as you work the creature, a surge of arousal flowing through your body.  You rub your thighs together and moan into the creatures cock, desperate to be touched by something.");
    ...
    private oozeRapesYouVaginally(): void {
        ...
        if (this.player.cor <= 20) {
            ...
            if (this.player.cockTotal() > 0) {
                ...
                if (this.player.cockTotal() > 1) {
                    ...
                    if (this.player.cockTotal() > 2)
                        this.outx("switching between cocks as you rapidly stroke yourself in time with the creature's movements.  ");
                    else
                        this.outx("rapidly stroking one cock with each hand, working yourself in time with the creature's movements.  ");
                }
                // Single dick
                else
                    this.outx(
            ...
            this.outx("After a short time the creature penetrates you fully again and stops, and you realize that at some point you had begun moving your hips with it.  You gasp in surprise as the slime's cock begins churning inside of you, massaging you from the inside in a uniquely pleasurable fashion.\n\n");
            this.outx("It doesn't take much of this to bring you to orgasm.  Your entire body shudders and you arch your back as shocks of pleasure run through you.  The creature pulls you tightly against it and swells just slightly as its movement inside you slows and then stops, then swells at the tip for just a second before releasing a flood of thin, cool fluid inside of you.  It forces its way around the creature's cock and spills out of you.  ");
            // Dicks again!
            if (this.player.cockTotal() > 0) {
                // Multi
                if (this.player.cockTotal() > 1)
        ...
        // standard result
            ...
            // Loose
                ...
                this.outx("It finally begins to penetrate and you grab at the grass beneath you, delighted that its cock seems large enough to fill you");
                // SLUT!
    ...
    public rapeOozeWithMilk(): void {
        ...
        if (this.player.gender < 1)
            this.outx("gripping the ground as the slime has left you nothing to do; adding your hands to the mix feels like it'd be an insult to the creature's expert work.  ");
        // if duder
        else if (this.player.gender == 1)
            this.outx("Your hands grip the ground as the slime has left you nothing to do; adding your fingers to the mix feels like it'd be an insult to the creature's expert work.  ");
        // [if female or Herm-
        else
            this.outx(
        ...
        this.outx("Unfortunately, you seem to have done a little too much to it in your battle before. The ooze slides off, leaving you hanging on the edge of orgasm. Deciding you just won't stand for this, you scoop up most the mess left by the monster and use it as a masturbation aid, achieving sweet release by ");
        if (this.player.gender == 1)
            this.outx("feverishly jacking yourself off with it.  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you release your cum into it.  You drop most of its bulk ");
        // [if female-
        else if (this.player.gender == 2)
            this.outx(
        ...
        this.outx("Now empty, you leave the ooze in the pile it's in and walk away, feeling somewhat disappointed by the lackluster ending of it all, but overall satisfied with the fact that you've nursed.\n\n");
