...
export class Lumi extends BaseContent {
    public lumiEncounter(): void {
        ...
        if (this.flags[kFLAGS.LUMI_MET] == 0) {
            ...
            this.outx("You spot an anomaly in the barren wastes; a door that seems to be standing out in the middle of nowhere. Somehow, you figure that it must lead to some other part of the world, and the only reason it's here is because you can't get to where the door should be right now.\n\n");
            this.outx("Do you open it?");
            this.doYesNo(this.lumiLabChoices, this.camp.returnToCampUseOneHour);
        } else {
            // placeholder text for outside the cathedral
            this.outx("You spot the door standing in the middle of nowhere again, and you guess that it will lead you back to Lumi's laboratory.  It swings open easily...");
    ...
    private lumiPitchGobboAle(): void {
        this.spriteSelect(37);
        this.clearOutput();
        this.outx("You point at the flagon. \"<i>Oh? Oh thats Lumi's... actually no, dat tispsy stuff for 20 gems. You'll like if you want to be like Lumi. Do you like it?</i>\"\n\n");
    ...
    private lumiEnhanceGo(itype: ItemType): void {
        ...
        this.outx("", true);
        this.outx("Lumi grabs the item from you and runs over to her table, stopping for only a second to put her apron on.  ");
        // start list of possible enhancement texts
        this.temp = Lumi.rand(3);
        if (itype == this.consumables.GLDSEED)
            this.outx("She fiddles with it, coating it in exotic powders before she tosses the whole mess onto a hotplate.  It explodes, knocking the goblin flat on her ass.  She sits bolt upright and snatches up the now-glowing seed with a gloved hand.\n\n");
        else if (itype == this.consumables.FOXJEWL)
            this.outx(
            ...
            this.outx("She fiddles with it, coating it in exotic powders before she tosses the whole mess onto a hotplate.  It explodes, knocking the goblin flat on her ass.  She sits bolt upright and snatches up the now-glowing fruit with a gloved hand.\n\n");
        else if (this.temp == 0)
            this.outx(
